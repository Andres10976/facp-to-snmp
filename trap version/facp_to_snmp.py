...
class SNMPManager:
    ...
    async def run(self):
        while True:
            ...
            asyncio.create_task(self.send_trap_with_resend(**message))
            self.queue.task_done()

    async def send_trap_with_resend(self, event: str, severity: int, timestamp: str, description: str):
        for attempt in range(RESEND_QUANTITY):
            await self.send_trap(event, severity, timestamp, description)
            if attempt < RESEND_QUANTITY - 1:
                await asyncio.sleep(BASE_INTERVAL)

    async def send_trap(self, event: str, severity: int, timestamp: str, description: str):
        try:
            ...
            if error_indication:
                logging.error(f"SNMP Trap failed: {error_indication}")
            elif error_status:
                logging.error(f"SNMP Trap failed: {error_status.prettyPrint()} at {error_index and var_binds[int(error_index) - 1][0] or '?'}")
            else:
                logging.info(f"SNMP Trap sent: {event}")
        except Exception as e:
            logging.error(f"Error sending SNMP Trap: {e}")
